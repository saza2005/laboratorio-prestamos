...
const setupPath = 'scripts/e2e/setup-flow-ret2-full-return-v1.mjs'
const testPath = 'tests/e2e/mutating/full-return-ret2.actual.spec.ts'
const cleanupPath = 'scripts/e2e/cleanup-flow-ret2-full-return-v1.mjs'
const runnerPath = 'scripts/e2e/run-flow-ret2-full-return-v1.mjs'
...
proof(process.cwd() === root && args.has('--confirm-e2e') && args.has('--flow=RET2') && args.has('--execute-full-return'), 'PRECONDITION', 'AUTHORIZATION')
...
proof(pre.status === 'PRISTINE' && pre.setupAttempt === 0 && pre.cleanupAttempt === 0, 'PRECONDITION', 'TRACKER_PRISTINE')
proof(Array.isArray(priorHistory.records) && priorHistory.records.length === 0, 'PRECONDITION', 'HISTORY_EMPTY')
...
const manifest = { runner_version: 'ret2-full-return-v1', flow: 'RET2', setup_sha256: await sha256File(setupPath), test_sha256: await sha256File(testPath), cleanup_sha256: await sha256File(cleanupPath), runner_sha256: await sha256File(runnerPath), timestamp_local: new Date().toISOString() }
...
try {
  ...
  setupComplete = true
  await fs.writeFile(protocolPath, '', { flag: 'wx', mode: 0o600 }).catch(() => failure('PRECONDITION', 'PROTOCOL_AUDIT_ALREADY_EXISTS'))
  ...
  const returns = await read(admin, 'returns', 'id,loan_id,notes', q => q.eq('loan_id', current.fixture.loanId).eq('notes', current.ownership), 'REMOTE_POSTSTATE_VERIFY', 'RETURN_ROW_EXACT')
  ...
  execFileSync(process.execPath, ['--env-file=.env.e2e', cleanupPath, '--confirm-e2e', '--flow=RET2', '--execute'], { cwd: root, stdio: 'inherit', env: { ...process.env, E2E_MUTATING_CONFIRM: 'RET2-CLEANUP' } })
  ...
  proof(terminal.status === 'CONSUMED_CLEAN_NO_ACTIVE_FIXTURE' && finalHistory.records.length === priorHistory.records.length + 1, 'TERMINAL_STATE', 'CONSUMED_CLEAN_HISTORY')
  console.log('L1_FLOW_RET2: PASS')
} catch (error) {
