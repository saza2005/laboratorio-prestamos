...
function directEnvironmentProperty(owner, name) {
  const entriesCall = one(all(owner, (node) => ts.isCallExpression(node) && callName(node) === 'entries'), `environment_entries:${name}`)
  ...
  return one(entriesCall.arguments[0].properties.filter((property) => propertyName(property.name) === name), `environment_direct_property:${name}`)
}
...
const cleanupCallStatements = statements(runnerTry.tryBlock).filter((statement) =>
  all(statement, (node) => ts.isCallExpression(node) && callName(node) === 'runCleanupOnce').length === 1)
const cleanupCallStatement = one(cleanupCallStatements, 'cleanup_invocation_statement')
const cleanupInvocation = one(all(cleanupCallStatement, (node) => ts.isCallExpression(node) && callName(node) === 'runCleanupOnce'), 'cleanup_invocation')
...
for (const owner of cleanupFailureFunctions) {
  const failures = directIfs(owner.body, (node) =>
...
const replacements = {
  ...
  CLEANUP_FAILURE_TERMINATION: uniqueProof('CLEANUP_FAILURE_TERMINATION', () => {
    for (const owner of cleanupFailureFunctions) requireProof(directIfs(owner.body, (node) =>
...
console.log('FLOW_L1_B_DELIVERY_V18_CONTRACT: PASS')
