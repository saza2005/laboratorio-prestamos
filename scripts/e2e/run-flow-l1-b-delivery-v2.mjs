...
const cleanupV2 = 'scripts/e2e/cleanup-l1-delivery-fixture-v2.mjs'
...
try {
  ...
  const outcome = await waitForDeliveryOutcome()
  if (outcome === 'attempted') deliveryAttempted = true
  if (outcome === 'result') writeEvent('CLEANUP_REQUIRED')
  ...
  const childCode = await waitForChild().catch(() => 1)
  if (canRecover()) {
  ...
  console.error('L1_DELIVERY_RUNTIME: FAIL\nCATEGORY: ' + (error instanceof Error && /^[a-z0-9_-]+$/i.test(error.message) ? error.message : (childCode === 0 ? 'runtime_failed' : 'child_exit_ambiguous')))
  process.exit(1)
}
...
async function waitForDeliveryOutcome() {
  ...
  while (Date.now() < deadline) {
    ...
    if (state === 'DELIVERY_SUBMIT_ATTEMPTED') return 'attempted'
...
async function runCleanupOnce() {
  ...
  execFileSync(process.execPath, ['--env-file=.env.e2e', cleanupV2, '--confirm-e2e', '--flow=FLOW-L1', '--execute'], { stdio: 'inherit', env: { ...process.env, E2E_MUTATING_CONFIRM: 'FLOW-L1-CLEANUP' } })
  clearRecoveryState()
}
