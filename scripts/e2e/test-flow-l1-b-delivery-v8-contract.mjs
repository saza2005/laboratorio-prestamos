...
const recovery = scope(runner, '} catch (error) {', 'error_recovery')
sequence(lifecycle, [
...
assert(recovery.includes('catch'), 'recovery_is_error_scope')
assert(lifecycle !== recovery, 'audit_scopes_distinct')
...
const launch = "spawn('npx', ['playwright', 'test', actualSpec"
assert(runner.includes(launch), 'positive_npx_playwright_launch')
...
console.log('PASS:F3IY_STATIC_CONTRACT')
