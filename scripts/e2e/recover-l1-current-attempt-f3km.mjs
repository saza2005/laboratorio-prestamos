...
function fail(code) {
  console.error(`L1_F3KM_RECOVERY: FAIL_CLOSED (${code})`)
  process.exitCode = 1
...
requireProof(state.flow === 'FLOW-L1' && state.requestCreateAttempt === 1 && state.approvalAttempt === 1 && state.deliveryAttempt === 0 && state.cleanupAttempt === 1, 'current_attempt_tracker_invalid')
...
requireProof(artifactHashes.runner === expected.runner && artifactHashes.cleanup === expected.cleanup && artifactHashes.f3ig === expected.f3ig && artifactHashes.recovery === expected.recovery, 'artifact_hash_mismatch')
...
const record = {
  ...
  recovery: { tool: 'recover-l1-current-attempt-f3km.mjs', result: 'MUTATION_COMPLETE_POST_VERIFY_PASS', consumed: true },
  artifactHashes,
...
console.log(`L1_F3KM_RECOVERY: PASS ${classification.name} DELIVERY=${classification.delivery} REMOTE_POST_VERIFY=PASS HISTORY_APPEND=PASS TRACKER_POSTSTATE=PRISTINE`)
