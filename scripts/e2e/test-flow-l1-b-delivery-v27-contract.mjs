...
function directConditionalReturnBranch(owner, text, label) {
  requireProof(ts.isFunctionLike(owner) && owner.body && ts.isBlock(owner.body), `branch_owner:${label}`)
  const statement = one(statements(owner.body).filter((candidate) =>
    ts.isReturnStatement(candidate) && ts.isConditionalExpression(candidate.expression) && literal(candidate.expression.whenFalse) === text), label)
  return { statement, branch: statement.expression.whenFalse }
}
...
const eventMonitorProofs = {
  ...
  EVIDENCE_MONOTONICITY: uniqueProof('EVIDENCE_MONOTONICITY', () => {
    requireProof(all(waitForEventOwner, (node) => ts.isBinaryExpression(node) && node.operatorToken.kind === ts.SyntaxKind.GreaterThanToken &&
      ts.isIdentifier(node.left) && node.left.text === 'stateIndex' && ts.isIdentifier(node.right) && node.right.text === 'expectedIndex').length === 2, 'monotonic_index_comparison')
  }),
...
const preserved = {
  ...
  UNKNOWN_ZERO_MUTATION: uniqueProof('UNKNOWN_ZERO_MUTATION', () => {
    const unknown = directConditionalReturnBranch(classifyOwner, 'UNEXPECTED_OR_AMBIGUOUS_STRUCTURE', 'unknown_branch')
    requireProof(mutationCallsInBranch(unknown.branch).length === 0, 'unknown_zero_mutation')
  }),
...
console.log('FLOW_L1_B_DELIVERY_V27_CONTRACT: PASS')
