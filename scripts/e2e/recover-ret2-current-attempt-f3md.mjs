...
const fail = (code) => { console.error(`L1_F3MD_RECOVERY_FAILURE=${code}`); process.exit(1) }
...
proof(args.has('--confirm-e2e') && args.has('--flow=RET2') && args.has('--execute'), 'AUTHORIZATION')
...
console.log(`L1_F3MD_RET2_RECOVERY: PASS CLASS=${classification} BUSINESS=${business} REMOTE_POST_VERIFY=PASS HISTORY_APPEND=PASS TRACKER_POSTSTATE=CONSUMED_CLEAN_NO_ACTIVE_FIXTURE`)
