...
const protocolAuditPath = path.resolve('.e2e-state/runtime/l1-b-protocol-audit.jsonl')
...
const browserOwnedEventOrder = eventOrder.slice(0, eventOrder.indexOf('CLEANUP_REQUIRED'))
...
try {
  ...
  appendProtocolAudit('RUNNER', 'RUNNER_ACTION_ARMED_EVIDENCE_PROVEN')
  ...
  appendProtocolAudit('RUNNER', 'RUNNER_AUTHORIZATION_WRITE_STARTED')
  writeEvent('DELIVERY_SUBMIT_AUTHORIZED')
  appendProtocolAudit('RUNNER', 'RUNNER_AUTHORIZATION_WRITE_COMPLETED')
  ...
  writeEvent('COMPLETE')
  console.log('L1_DELIVERY_RUNTIME_V5: PASS')
...
function initializeProtocolAudit() {
  if (fs.existsSync(protocolAuditPath)) {
    fs.renameSync(protocolAuditPath, `${protocolAuditPath}.history-${Date.now().toString(36)}`)
  }
  fs.writeFileSync(protocolAuditPath, '', { mode: 0o600 })
  appendProtocolAudit('RUNNER', 'RUNNER_PROTOCOL_AUDIT_INITIALIZED')
}
function appendProtocolAudit(actor, marker) {
  fs.appendFileSync(protocolAuditPath, JSON.stringify({ sequence: `${Date.now()}-${process.hrtime.bigint()}`, actor, marker }) + '\n', { mode: 0o600 })
}
...
async function waitForEvent(expected) {
  ...
  while (Date.now() < deadline) {
    ...
    if (state === expected) return { state, observed: true }
    const expectedIndex = eventOrder.indexOf(expected)
    const stateIndex = eventOrder.indexOf(state)
    // F3IG writeEvent enforces strict forward order. A later browser-owned
    // state therefore proves earlier milestones even if polling missed them.
    if (browserOwnedEventOrder.includes(state) && stateIndex > expectedIndex) return { state, observed: false, implied: true }
    if (state === 'ABORT' || (state && stateIndex > expectedIndex)) fail('delivery_event_unproven_' + expected.toLowerCase())
