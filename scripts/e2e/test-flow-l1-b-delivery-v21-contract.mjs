...
function directCallFromStatement(statement, name, label) {
  ...
  if (ts.isExpressionStatement(statement) && ts.isCallExpression(statement.expression)) {
    candidates = [statement.expression]
...
function directEnvironmentProperty(owner, name) {
  ...
  const entriesCall = one(returnStatement.expression.arguments.filter((argument) =>
    ts.isCallExpression(argument) && callName(argument) === 'entries'), `environment_entries:${name}`)
...
const cleanupCallStatements = statements(runnerTry.tryBlock).filter((statement) => {
  if (ts.isExpressionStatement(statement) && ts.isCallExpression(statement.expression)) {
    return callName(statement.expression) === 'runCleanupOnce'
  }
...
console.log('FLOW_L1_B_DELIVERY_V21_CONTRACT: PASS')
