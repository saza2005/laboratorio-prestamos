...
const fail = (code) => { console.error(`RET2_CLEANUP: FAIL_CLOSED (${code})`); process.exit(1) }
...
proof(process.env.E2E_MUTATING_CONFIRM === 'RET2-CLEANUP', 'cleanup_confirmation_required')
try { process.loadEnvFile('.env.e2e') } catch { fail('env_e2e_load_failed') }
const state = JSON.parse(await fs.readFile(trackerPath, 'utf8'))
proof(state.flow === 'RET2' && state.status === 'CLEANUP_REQUIRED' && state.cleanupAttempt === 0 && state.fixture?.itemCreatedByAttempt, 'tracker_invalid')
...
const returns = await read('returns', 'id,loan_id,notes', q => q.eq('loan_id', state.fixture.loanId).eq('notes', state.ownership))
...
if (returns.length || returnItems.length) {
  classification = 'FULLY_RETURNED_OWNED'
  proof(returns.length === 1 && returnItems.length === 1, 'return_graph_ambiguous')
  proof(loans.length === 1 && loans[0].status === 'returned', 'returned_loan_invalid')
  proof(loanItems.length === 1 && loanItems[0].quantity === 1 && loanItems[0].returned_quantity === 1 && loanItems[0].damaged_quantity === 0 && loanItems[0].missing_quantity === 0, 'returned_loan_item_invalid')
  ...
  proof(item[0].stock_available === 1, 'returned_stock_invalid')
  proof(movements.length === 2 && movements.some((m) => m.reference_table === 'loans' && m.reference_id === state.fixture.loanId && m.movement_type === 'loan_out' && m.quantity === 1) && movements.some((m) => m.reference_table === 'returns' && m.reference_id === returns[0].id && m.movement_type === 'return_ok' && m.quantity === 1), 'movement_graph_invalid')
} else {
  proof(loans.length === 1 && loans[0].status === 'active' && loanItems.length === 1 && loanItems[0].quantity === 1 && item[0].stock_available === 0, 'pre_return_graph_invalid')
  proof(movements.length === 1 && movements[0].reference_table === 'loans' && movements[0].reference_id === state.fixture.loanId && movements[0].movement_type === 'loan_out' && movements[0].quantity === 1, 'pre_return_movement_invalid')
}
...
history.records.push({ version: 1, state: 'CONSUMED_CLEAN_NO_ACTIVE_FIXTURE', flow: 'RET2', outcome: classification, cleanup: 'PASS', secretFields: 0 })
...
console.log(`RET2_CLEANUP: PASS CLASS=${classification} POST_VERIFY=PASS HISTORY_APPEND=PASS TRACKER=CONSUMED_CLEAN_NO_ACTIVE_FIXTURE`)
