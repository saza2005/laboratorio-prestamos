...
const expected = {
  ...
  f3ig: '1ef535514d36185f95657f69af38dede0b2cacacbd1fb61954464a35528fd24f',
}

function fail(code) {
  console.error(`L1_F3LJ_RECOVERY: FAIL_CLOSED (${code})`)
  process.exitCode = 1
...
requireProof(artifactHashes.runner === expected.runner && artifactHashes.cleanup === expected.cleanup && artifactHashes.f3ig === expected.f3ig, 'artifact_hash_mismatch')
...
const record = {
  ...
  recovery: { tool: 'recover-l1-current-attempt-f3lj.mjs', result: 'MUTATION_COMPLETE_POST_VERIFY_PASS', consumed: true },
  artifactHashes,
...
console.log(`L1_F3LJ_RECOVERY: PASS ${classification.name} DELIVERY=${classification.delivery} REMOTE_POST_VERIFY=PASS HISTORY_APPEND=PASS TRACKER_POSTSTATE=PRISTINE`)
