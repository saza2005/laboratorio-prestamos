...
const fail = (code) => { console.error(`RET2_SETUP_V2: FAIL_CLOSED (${code})`); process.exit(1) }
...
proof(tracker.flow === 'RET2' && tracker.status === 'CONSUMED_CLEAN_NO_ACTIVE_FIXTURE' && tracker.setupAttempt === 1 && tracker.cleanupAttempt === 1 && tracker.remoteWriteProven === true, 'prior_attempt_not_clean')
proof(Array.isArray(history.records) && history.records.length === 1 && history.records[0].state === 'CONSUMED_CLEAN_NO_ACTIVE_FIXTURE', 'history_attempt1_not_preserved')
...
const attemptId = `RET2-2-${Date.now().toString(36)}-${crypto.randomBytes(5).toString('hex')}`
