...
-- The Supabase-managed default ACL for supabase_admin is intentionally not
-- modified here because the local migration runner cannot alter that role.
-- Every future application function must explicitly revoke EXECUTE from
-- PUBLIC and anon in the same migration that creates or replaces it.
