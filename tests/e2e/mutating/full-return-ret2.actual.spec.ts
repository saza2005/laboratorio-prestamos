...
async function audit(marker: string) {
  sequence += 1
  await fs.appendFile(auditPath, JSON.stringify({ sequence, timestamp: new Date().toISOString(), actor: 'F3IG_RET2', marker }) + '\n', { mode: 0o600 })
}
function escaped(value: string) { return value.replace(/[.*+?^${}()|[\]\\]/g, '\\$&') }

test('RET2 full return bulk quantity one', async ({ page }, testInfo) => {
  ...
  let submissions = 0
  page.on('request', (request) => { if (request.method() === 'POST' && request.url().includes('/_next/')) submissions += 1 })
  ...
  const responsePromise = page.waitForResponse((response) => response.request().method() === 'POST' && response.url().includes('/_next/'))
